...
// Headline: OASIS-scoped vs AgentCash head-to-head win-rate. Plus gate contribution (scoped vs off).
...
const arms = { scoped: load("oasis_scoped.json"), off: load("oasis_off.json"), agentcash: load("agentcash.json"), live: load("live.json") };
...
console.log(`\nHead-to-head — OASIS(scoped) vs AgentCash, #1 correctness over ${N} queries:`);
console.log(`  OASIS #1 correct:     ${(100 * agg.scoped.p1).toFixed(1)}%`);
console.log(`  AgentCash #1 correct: ${(100 * agg.agentcash.p1).toFixed(1)}%`);
console.log(`  both right: ${both}  neither: ${neither}  OASIS-only: ${oasisOnly}  AgentCash-only: ${acOnly}`);
const decisive = oasisOnly + acOnly;
console.log(`  → on the ${decisive} decisive queries: OASIS wins ${oasisOnly} (${(100*oasisOnly/decisive||0).toFixed(0)}%), AgentCash wins ${acOnly} (${(100*acOnly/decisive||0).toFixed(0)}%)`);
console.log(`  → OASIS ">=" AgentCash on #1: ${(100 * (both + neither + oasisOnly) / N).toFixed(1)}% of queries (win-or-tie)`);
...
console.log(`\nwrote score.json (incl. the ${acOnly} AgentCash-only losses for attribution)`);
