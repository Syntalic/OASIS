...
function inferSource(fqn) {
    if (fqn.startsWith("mpp-catalog/"))
        return "mpp-catalog";
    if (fqn.startsWith("x402scan/"))
        return "x402scan";
    if (fqn.startsWith("mppscan/"))
        return "mppscan";
    return "pay-skills";
}
function railProtocols(endpoints) {
    const rails = new Set();
    for (const ep of endpoints) {
        for (const r of ep.payment.rails)
            rails.add(r.protocol);
    }
    return [...rails].sort();
}
function minPriceUsd(endpoints) {
    const prices = endpoints
        .map((e) => e.payment.price_usd)
        .filter((p) => p != null);
    return prices.length ? Math.min(...prices) : undefined;
}
export function buildProviderRecords(endpoints, paySkillsProviders = []) {
    ...
    for (const [fqn, eps] of byFqn) {
        ...
        const categories = [
            ...new Set([paySkills?.category, ...eps.map((e) => e.category)].filter(Boolean)),
        ];
        const richDescriptions = eps
            .map((e) => e.description)
            .filter((d) => Boolean(d && d.length > 24));
        const inferredDescription = richDescriptions[0] ??
            eps.find((e) => e.summary.length > 20 && !e.summary.startsWith("GET /"))?.summary;
        const tagSummary = [
            ...new Set(eps.flatMap((e) => e.tags ?? [])),
        ].slice(0, 8);
        const inferredUseCase = paySkills?.use_case ??
            (tagSummary.length
    ...
    for (const p of draft) {
        ...
        const ranked = peers
            ...
            .map((x) => x.fqn);
        if (ranked.length)
            p.alternatives = ranked;
    }
    return draft.sort((a, b) => a.fqn.localeCompare(b.fqn));
}
export function enrichEndpointsWithProviders(endpoints, providers) {
