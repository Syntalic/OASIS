...
export const INTENT_MATCHERS = {
    "ai.image_generate": (ep) => {
        ...
        return (/image|diffusion|sdxl|flux|dall|fal-ai/i.test(text) &&
            /generat|create|render|text.?to.?image/i.test(full));
    },
    "ai.llm_complete": (ep) => /chat.?completion|\/v1\/chat\/completions|llm.?complet|text.?generation/i.test(corpus(ep)) && !/embeddings?/i.test(corpus(ep)),
    "ai.speech_to_text": (ep) => {
        const text = corpus(ep);
        return (/speech.?to.?text|transcri|whisper|\/stt|speech.?recognition/i.test(text) &&
            !/text.?to.?speech|tts/i.test(text));
    },
    "ai.text_to_speech": (ep) => /text.?to.?speech|texttospeech|\/tts|voice.?synthesis/i.test(corpus(ep)),
    "ai.embeddings": (ep) => /\/embeddings|text.?embeddings?|vector.?embed|sentence.?embed/i.test(corpus(ep)),
    "ai.web_research": (ep) => /perplexity|sonar|grounded.?research|web.?research|cited.?web|live.?research/i.test(corpus(ep)),
    "ai.document_extract": (ep) => {
        const text = corpus(ep);
        return (/document.?extract|pdf.?pars|table.?extract|structured.?document|invoice.?extract/i.test(text) ||
            (/\/extract$/i.test(ep.path) &&
                /document|pdf|table|structured|invoice|scan/i.test(text)));
    },
    "analyst.inflation_tracker": (ep) => /\/analyst\/inflation|price.?inflation|category.?inflation|inflation.?trend/i.test(corpus(ep)),
    "cloud.domains": (ep) => /domain\/renew|domain.?renew|domain.?regist|register.?domain|manage.?dns/i.test(corpus(ep)),
    "comms.agent_inbox": (ep) => /\/v0\/inboxes$/i.test(ep.path) && ep.method === "POST",
    "comms.send_email": (ep) => {
        const text = corpus(ep);
        return ((/\/send$/i.test(ep.path) || /\/api\/send/i.test(ep.path)) &&
    ...
    "comms.voice_call": (ep) => {
        const text = corpus(ep);
        return (/\/api\/call|voice.?call|outbound.?call|phone.?call|ai.?call/i.test(text) &&
    ...
    "data.email_validate": (ep) => /email.?(reputation|validat|verif|deliverab)|validate.?email|disposable.?email/i.test(corpus(ep)),
    "data.exchange_rates": (ep) => /exchange.?rate|forex|currency.?convert|fx.?rate|foreign.?exchange/i.test(corpus(ep)),
    "data.ip_lookup": (ep) => /ip.?(intelligence|lookup|geoloc)|geo.?locate.?ip|threat.?intel.?ip/i.test(corpus(ep)),
    "data.job_search": (ep) => {
        const text = corpus(ep);
        return (/\/v1\/active-ats|\/active-ats/i.test(ep.path) ||
            /job.?search|active.?ats|active.?jb|job.?board|job.?listing|hiring.?feed/i.test(text));
    },
    ...
    "data.translate_text": (ep) => {
        const text = corpus(ep);
        return (/translate|translation|machine.?translat|\/deepl\/|localiz/i.test(text) &&
            !/speech|transcri|whisper/i.test(text));
    },
    "data.weather_forecast": (ep) => {
        const text = corpus(ep);
        return (/weather|forecast|current.?weather|air.?quality/i.test(text) &&
            !/tripadvisor|travel.?review/i.test(text));
    },
    "data.web_scrape": (ep) => {
        const text = corpus(ep);
        return (/scrape|web.?scrap|crawl.?url|fetch.?page/i.test(text) &&
            !/proxy|storage|interest|markdown|screenshot/i.test(text));
    },
    "data.whois_lookup": (ep) => {
        const text = corpus(ep);
        return ((/whois|dns.?record|nameserver|mx.?record|registrar/i.test(text) &&
            /lookup|dns|whois/i.test(text)) ||
            /dns.?lookup|lookup.?dns.?and.?whois/i.test(text)) && !/domain.?renew|domain.?regist/i.test(text);
    },
    ...
    "finance.stock_quote": (ep) => {
        const text = corpus(ep);
        if (/\/alphavantage\/global-quote|global.?quote/i.test(text))
            return true;
        return (/time.?series.?intraday|stock.?quote|equity.?quote/i.test(text) ||
            (/stock|equity|ticker/i.test(text) && /quote|intraday|market.?data/i.test(text)));
    },
    "finance.token_balance": (ep) => {
        const text = corpus(ep);
        return (/token.?balance|wallet.?balance|erc.?1155|holdings/i.test(text) &&
            !/nft|ownerof|proxy|json.?rpc|mainnet/i.test(text));
    },
    "maps.places": (ep) => {
        const text = corpus(ep);
        return (/places:search|\/places\/search|searchnearby|searchtext/i.test(ep.path) ||
    ...
    "comms.send_sms": (ep) => {
        const text = corpus(ep);
        return (/\/text$/i.test(ep.path) ||
            (/send.?sms|outbound.?sms|text.?message|sms.?verif/i.test(text) &&
                !/speech|transcri|whisper/i.test(text)));
    },
    "realestate.property_lookup": (ep) => {
        const text = corpus(ep);
        return (/listings\/sale|property.?list|rental.?list|homes?.?for.?sale|real.?estate.?search/i.test(text) ||
            (/property|rental|rentcast/i.test(text) && /list|search|lookup|estimate/i.test(text)));
    },
    "search.web": (ep) => {
        const text = corpus(ep);
        if (/\/search$/i.test(ep.path) &&
            /serper|google.?search|organic.?search|organic.?results|serp/i.test(text)) {
            return true;
        }
        return ((/\/search/i.test(ep.path) ||
            /\/api\/serper\/search|organic.?search|web.?search|google.?search|serp/i.test(text)) &&
            !/perplexity|sonar|smart.?money|influencer|person\/search|deal.?finder|image.?search|news.?search/i.test(text));
    },
    "shop.compare_price": (ep) => /\/shopper\/best-price|best.?price|price.?compar|compare.?price|cheapest.?price|cross.?retailer/i.test(corpus(ep)),
    "shop.find_deals": (ep) => /\/shopper\/deal-finder|deal.?finder|find.?deal|discounted.?product|on.?sale|clearance/i.test(corpus(ep)),
    "shop.price_drop_alert": (ep) => /\/shopper\/price-drop|price.?drop|price.?alert|price.?decrease|recent.?markdown/i.test(corpus(ep)) || /\/v1\/shopper\/price-drop-alert/i.test(ep.path),
    "shop.track_price_history": (ep) => /\/shopper\/price-history|price.?history|historical.?pric|track.?price|price.?trend/i.test(corpus(ep)),
    ...
    "travel.place_reviews": (ep) => {
        const text = corpus(ep);
        return (/\/api\/v1\/location|tripadvisor|place.?review|travel.?review|hotel.?review/i.test(text) ||
            (/\/location/i.test(ep.path) &&
                /review|hotel|restaurant|attraction|nearby/i.test(text)));
    },
    "web.markdown_extract": (ep) => {
        const text = corpus(ep);
        if (/serper-scrape|serper.?scrape/i.test(`${ep.origin} ${text}`) &&
            /markdown|clean.?text/i.test(text)) {
            return true;
        }
        return (/markdown|url.?to.?markdown|page.?to.?markdown|webpage.?to.?markdown|clean.?text/i.test(text) ||
