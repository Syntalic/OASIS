...
export function isGenericEndpoint(ep) {
...
function entityInputTokens(entity) {
 ...
 */
export function intentRelevanceBonus(ep, intent) {
    let bonus = 0;
 ...
 */
export function scoreEndpointNeutral(ep, intent) {
    if (isGenericEndpoint(ep))
        return -100;
    let score = 0;
    if (ep.description && ep.description.length > 20)
        score += 3;
    if (ep.inputs?.length)
    ...
    if (ep.summary.length > 12)
        score += 1;
    if (intent)
        score += intentRelevanceBonus(ep, intent);
    return score;
}
export function rankEndpointsNeutral(endpoints, max = 12, intent) {
...
//# sourceMappingURL=score-endpoint.js.map