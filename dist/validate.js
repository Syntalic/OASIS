...
export async function loadSchemas() {
    ...
    addFormats(ajv);
    const [endpointSchema, capabilitySchema, providerSchema, indexSchema] = await Promise.all([
...
export async function validateBundle(bundle) {
    ...
    try {
        ...
        console.error(`referential-integrity gate skipped: ${err.message}`);
    }
    return issues;
}
// ---------------------------------------------------------------------------
...
async function loadEntityVocab() {
    const raw = await readFile(path.join(PACKAGE_ROOT, "spec", "entity-vocab.json"), "utf8");
    const parsed = JSON.parse(raw);
    return new Set(Object.keys(parsed.entities ?? {}));
}
async function loadIntentSources(intentsDir) {
...
function checkPorts(intentId, ports, side, vocab, issues) {
    let checked = 0;
 ...
 */
export async function checkReferentialIntegrity(sources, vocab) {
    const issues = [];
    ...
    for (const intent of sources) {
        for (const link of intent.links ?? []) {
            ...
            if (!produced.size || !consumed.size)
                continue; // not enough info to lint
...
export async function runReferentialIntegrity(intentsDir = path.join(PACKAGE_ROOT, "ontology", "intents")) {
