...
export function inferQueryDomains(query) {
...
function capabilityDomain(cap) {
    if (cap.facets?.domain)
        return cap.facets.domain;
 ...
 */
export function coarseCapabilityAllowlist(query, bundle) {
    const queryDomains = inferQueryDomains(query);
    if (queryDomains.size === 0)
        return null; // nothing inferred → no restriction
    const allowed = new Set();
    for (const cap of curatedCapabilitiesForSearch(bundle)) {
        if (queryDomains.has(capabilityDomain(cap))) {
            allowed.add(cap.id);
        }
    }
...
function mergeKeywordAndVector(keywordHits, vectorHits, bundle, limit, fusion, capabilityAllowlist) {
    ...
    const capAllowed = (capId) => capabilityAllowlist === null ||
        capId === undefined ||
        capabilityAllowlist.has(capId);
    for (let i = 0; i < keywordHits.length; i++) {
        const hit = keywordHits[i];
        if (hit.kind === "capability" && !capAllowed(hit.capability_id))
            continue;
    ...
    for (let i = 0; i < vectorHits.length; i++) {
        ...
        if (kind === "capability" && !capAllowed(id))
            continue;
...
export async function searchHybrid(query, bundle, lanceDir, limit = 10, options = {}) {
    ...
    return mergeKeywordAndVector(keywordHits, vectorHits, bundle, limit, { keywordWeight, vectorWeight }, capabilityAllowlist);
}
