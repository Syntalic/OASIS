...
const CURATED_CAPABILITY_IDS = new Set(CURATED_INTENT_IDS);
const GENERIC_SUMMARY = /^(authenticate|prove action|delete a memory|get mcp|api info|free health|purchase |buy )/i;
...
function stem(token) {
    ...
    if (token.endsWith("sses"))
        return token.slice(0, -2); // addresses -> address
 ...
 */
function compoundMatch(q, corpusTokens) {
    for (const c of corpusTokens) {
        if (c.length >= 4 && q.length >= c.length + 3 && q.endsWith(c))
            return true;
    }
    return false;
}
function scoreTokens(queryTokens, corpus) {
...
function phraseBoost(query, phrases) {
...
function intentIdBoost(query, intentId) {
 ...
 */
export function inferQueryFacets(query) {
    const q = query.toLowerCase();
    const out = {};
    ...
    for (const [entity, re] of ENTITY_CUES) {
        if (re.test(q)) {
            out.primary_entity = entity;
            break;
        }
    }
    for (const [entity, re] of OUTPUT_CUES) {
        if (re.test(q)) {
            out.output_entity = entity;
            break;
        }
    }
    const modality = [];
...
function consumesEntities(ports) {
    return new Set((ports ?? []).map((p) => p.entity));
}
function producesEntities(ports) {
    return new Set((ports ?? []).map((p) => p.entity));
}
/**
...
function facetMatchBoost(cap, inferred) {
    ...
    if (inferred.output_entity && produce.size > 0) {
        if (produce.has(inferred.output_entity))
            factor *= 1.18;
        else
            factor *= 0.7;
    }
    if (inferred.modality?.length && facets?.modality?.length) {
        ...
        if (inferred.modality.some((m) => capMods.has(m)))
            factor *= 1.1;
        else
            factor *= 0.8;
    }
 ...
 */
function negativeTermsDemotion(cap, query) {
...
export function searchIndex(query, endpoints, capabilities, limit = 10) {
    const queryTokens = tokenize(query);
    const inferred = inferQueryFacets(query);
    const hits = [];
    for (const cap of capabilities) {
        if (!CURATED_CAPABILITY_IDS.has(cap.id))
            continue;
        const phrases = [
            cap.label,
            cap.description,
        ...
        const boost = phraseBoost(query, phrases) +
