...
const ACTION_WORDS = /\b(send|find|get|lookup|enrich|scrape|call|price|register|upload|solve|transcribe|generate|compare|track|search|convert|validate|check|create|host|tip|fax|email|buy|sell|rent|deploy|analyze|monitor|fetch|extract|translate|summarize)\b/i;
...
function endpointExists(endpoints, ref) {
    ...
    for (const phrase of phrases) {
        ...
        for (const prefix of PARAPHRASE_PREFIXES) {
            for (const suffix of PARAPHRASE_SUFFIXES) {
                ...
                if (query.length < 12 || query.length > 120)
                    continue;
                generated.push({
                    id: `${slug(cap.id)}-${n + 1}`,
                    query,
...
export async function generateEvalQueries(bundle, options = {}) {
    const maxPerCap = options.maxPerCapability ?? 3;
    const generated = [...HAND_CURATED];
    const curatedIds = new Set(HAND_CURATED.map((q) => q.expect_intent).filter(Boolean));
    for (const cap of bundle.capabilities) {
        if (curatedIds.has(cap.id) && cap.id.startsWith("shop."))
            continue;
