...
describe("discovery benchmark", () => {
    ...
    it("full index beats endpoints-only on workflow discovery", async () => {
        ...
        assert.ok(full.intent_hit_at_1 > endpointsOnly.intent_hit_at_1, `intent@1: full=${full.intent_hit_at_1} endpoints-only=${endpointsOnly.intent_hit_at_1}`);
        assert.ok(full.workflow_hit_at_3 >= endpointsOnly.workflow_hit_at_3, `workflow@3: full=${full.workflow_hit_at_3} endpoints-only=${endpointsOnly.workflow_hit_at_3}`);
        assert.ok(full.workflow_mrr >= endpointsOnly.workflow_mrr, `workflow MRR: full=${full.workflow_mrr} endpoints-only=${endpointsOnly.workflow_mrr}`);
    });
    it("full index beats provider-only catalog search on endpoint@3", async () => {
        ...
        assert.ok(full.endpoint_hit_at_3 > providersOnly.endpoint_hit_at_3, `endpoint@3: full=${full.endpoint_hit_at_3} providers-only=${providersOnly.endpoint_hit_at_3}`);
    });
