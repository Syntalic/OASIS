...
import { evaluateResolveAccuracy, loadCuratedIntents, runResolveBenchmark, } from "./resolve-benchmark.js";
...
describe("resolve benchmark", () => {
    ...
    it("all curated intents resolve primary satisfies ref to an indexed endpoint", async (t) => {
        ...
        assert.equal(misses.length, 0, `expected all primary refs to resolve, missing: ${misses.map((m) => m.intent_id).join(", ")}`);
    });
