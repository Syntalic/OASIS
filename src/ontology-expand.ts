...
function aliasesFromText(...parts: Array<string | undefined>): string[] {
  const text = parts.filter(Boolean).join(" ").toLowerCase();
  const phrases = new Set<string>();
  for (const chunk of text.split(/[.,;]/)) {
    const trimmed = chunk.trim();
    if (trimmed.length >= 4 && trimmed.length <= 80) phrases.add(trimmed);
  }
...
function intentIdForMpp(serviceId: string, category?: string): string {
  const cat = slugify(category || "data");
  const name = slugify(serviceId);
  return `${cat}.${name}`;
}

export function expandOntologyFromProviders(
  curated: CapabilityIntent[],
  ...
  const mppByService = new Map<string, EndpointRecord[]>();
  for (const ep of endpoints) {
    if (!ep.provider_fqn?.startsWith("mpp-catalog/")) continue;
...
const KEYWORD_INTENTS: Array<{
  ...
  label: string;
  aliases: string[];
  match: (ep: EndpointRecord) => boolean;
}> = [
  {
  ...
  },
  {
    ...
    match: (ep) =>
      /image|diffusion|dall|flux|fal/i.test(`${ep.summary} ${ep.path}`) &&
  ...
  },
  {
    ...
    match: (ep) =>
      /influencer|creator/i.test(`${ep.summary} ${ep.provider_title ?? ""} ${ep.path}`),
  },
  {
    id: "web.screenshot",
