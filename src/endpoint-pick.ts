...
const PROVIDER_PATH_PREFERENCES: Record<string, RegExp[]> = {
...
const MPP_SERVICE_PATH_PREFERENCES: Record<string, RegExp[]> = {
...
function isGenericEndpoint(ep: EndpointRecord): boolean {
  if (GENERIC_SUMMARY.test(ep.summary)) return true;
  if (GENERIC_PATH.test(ep.path)) return true;
  if (/^lookup$/i.test(ep.summary) && ep.path.split("/").length > 4) return false;
  return false;
}

function useCaseOverlap(provider: PaySkillsProvider | undefined, ep: EndpointRecord): number {
  if (!provider?.use_case) return 0;
  ...
  for (const t of epTokens) {
    if (useTokens.has(t)) hits += 1;
  }
  return hits;
}

function pathPreferenceScore(
  ep: EndpointRecord,
  preferences: RegExp[],
): number {
  for (let i = 0; i < preferences.length; i++) {
    if (preferences[i].test(ep.path)) return preferences.length - i;
  }
  return 0;
}

export function scoreEndpointPrimary(
  ep: EndpointRecord,
  ...
  if (provider) {
    ...
    if (prefs) score += pathPreferenceScore(ep, prefs) * 5;
  }
  ...
  if (/^get /i.test(ep.summary) && ep.summary.length < 16) score -= 1;

  return score;
}

export function pickPrimaryEndpoints(
  ...
  return [...pool]
    .sort(
      (a, b) =>
        scoreEndpointPrimary(b, options.provider, options.serviceId) -
