...
describe("url-match", () => {
  it("parses full resource URLs", () => {
    const parsed = parseResourceUrl(
      "https://screenshotone.x402.paysponge.com/take",
      "GET",
    );
  ...
  it("matches expected endpoint refs", () => {
    const hit = parseResourceUrl(
      "https://stablecrypto.dev/api/coingecko/price",
      "POST",
    )!;
    const expected = {
