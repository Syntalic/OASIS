...
export const TASKS = [
  ...
  { q: "I need the raw HTML source of this competitor's landing page, not a picture of it", expect: "data.web_scrape", trap: "screenshot" },
  ...
  { q: "before I send my newsletter, weed out the addresses that will bounce", expect: "data.email_validate", trap: "send_email" },
  { q: "text my customer their appointment reminder", expect: "comms.send_sms", trap: "send_email/voice_call" },
  { q: "is this phone number actually a working mobile line", expect: "data.phone_validate", trap: "send_sms" },
  { q: "pull the vendor, total and date as structured fields from this scanned receipt", expect: "ai.document_extract", trap: "ocr" },
  { q: "just give me the plain text printed on this scanned page", expect: "data.ocr", trap: "document_extract" },
  { q: "what is one bitcoin worth in euros right this second", expect: "finance.crypto_spot_price", trap: "stock_quote/exchange_rates" },
  { q: "convert 500 US dollars into Japanese yen at today's rate", expect: "data.exchange_rates", trap: "crypto_spot_price" },
